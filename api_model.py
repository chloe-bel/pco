...
from functions import lignes_segm, best_skill, delete_training_data
...
@app.get("/my_algo")
def my_algo():
    return "API is up and running v1"
...
async def retrain():
    ...
    # si meilleures pour algo_1 et/ou algo_2 : 
        # ce sont les nouveaux modèles : doivent remplacer rfc_1 et rfc_2
        # importer les métriques et le nouveau modèle dans MLFlow
    ...
    # sinon: on garde rfc_1 et rfc_2

    # ENLEVER LES SEGMENTS UTILISES DES TABLES MONITORING
    
    #delete_training_data()  # Suppression des données après réentrainement
        #return {"message": "Réentrainement réussi et données supprimées.", "model_details": str(model)}
...
# requête curl pour tester l'endpoint /retrain
'''curl -X 'POST' \
